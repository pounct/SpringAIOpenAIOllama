...
//            Prompt zeroShotprompt = new Prompt(List.of(systemMessage, userMessage));
            ...
            ChatResponse chatResponse = ollamaChatModel.call(fewShotprompt);

            System.out.println(chatResponse.getResult().getOutput());

        } catch (Exception e) {
            System.err.println("Une erreur s'est produite lors de l'appel à l'API Ollama : " + e.getMessage());
            e.printStackTrace();
        }
    }
}
